...
void cli_allow_user(struct cli_def *cli, const char *username, const char *password) {
  struct unp *u, *n;
  if (!(n = static_cast<unp*>(malloc(sizeof(struct unp))))) {
...
void cli_unregister_single(struct cli_def *cli, struct cli_command *command) {
    ...
    if(command) {
        cli_command* my_next = command->next;
        cli_command* my_prev = command->previous;

        if(command->command_type == CLI_ANY_COMMAND or command->command_type == CLI_REGULAR_COMMAND) {

            if (command == cli->commands) {
                cli->commands = command->next;
            }

            if(my_next)
                my_next->previous = my_prev;

            if(my_prev)
                my_prev->next = my_next;

            // Unregister all child commands
            cli_free_command(cli, command);
        }
    }
}
...
static char *join_words(int argc, char **argv) {
  ...
  p = static_cast<char*>(malloc(len + 1));
  p[0] = 0;
...
int cli_match_filter(UNUSED(struct cli_def *cli), const char *string, void *data) {
  struct cli_match_filter_state *state = static_cast<cli_match_filter_state*>(data);
  int r = CLI_ERROR;
...
int cli_range_filter(UNUSED(struct cli_def *cli), const char *string, void *data) {
  struct cli_range_filter_state *state = static_cast<cli_range_filter_state*>(data);
  int r = CLI_ERROR;
